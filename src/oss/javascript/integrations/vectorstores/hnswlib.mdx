...
This guide provides a quick overview for getting started with `HNSWLib` [vector stores](/oss/integrations/vectorstores). For detailed documentation of all `HNSWLib` features and configurations head to the [API reference](https://reference.langchain.com/javascript/langchain-community/vectorstores/hnswlib/HNSWLib).
...
This guide will also use [OpenAI embeddings](/oss/integrations/embeddings/openai), which require you to install the `@langchain/openai` integration package. You can also use [other supported embeddings models](/oss/integrations/embeddings) if you wish.
